...
int main () {
    ...
    WNDCLASSA wcl; //структура 
        memset(&wcl, 0, sizeof(WNDCLASSA)); //заполнение структуры нулями 
        wcl.lpszClassName = "speedTester"; //назвали класс нашего окна
        wcl.lpfnWndProc = WindowProc; //процедура которая обрабатывает сообщения окну
    RegisterClassA(&wcl);

    hwnd_set = CreateWindow("speedTester", "Speed Tester", WS_OVERLAPPEDWINDOW, 500, 250, 330, 250, NULL, NULL, NULL, NULL); //создание окна 1

    hwnd = CreateWindow("speedTester", "Speed Tester", WS_OVERLAPPEDWINDOW, 10, 10, 1080, 720, NULL, NULL, NULL, NULL); //создание окна 2

    dc = GetDC(hwnd);

    HFONT fontRectangle = CreateFont(40, 15, 0, 0, FW_MEDIUM,FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_OUTLINE_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, FF_MODERN, "Font"); //создание шрифта

    ShowWindow(hwnd_set, SW_SHOWNORMAL); //показыывыает окно 1
    
    InitPointer(); //инициализация объекта указателя

    HWND difficulty_txt = CreateWindow("static", "Choose difficulty", WS_VISIBLE | WS_CHILD, 10, 10, 180, 20, hwnd_set, NULL, NULL, NULL);
    time_txt = CreateWindow("static", "Put the time in seconds", WS_VISIBLE | WS_CHILD, 10, 50, 180, 20, hwnd_set, NULL, NULL, NULL);
    mistakes_txt = CreateWindow("static", "Put the number of mistakes", WS_VISIBLE | WS_CHILD, 10, 90, 180, 20, hwnd_set, NULL, NULL, NULL);                                                            

    HWND difficulty = CreateWindow("combobox", "", WS_VISIBLE | WS_CHILD | CBS_DROPDOWNLIST, 200, 10, 100, 150, hwnd_set, Difficulty_id, NULL, NULL);
    ...
    HWND times = CreateWindow("edit", "120", WS_VISIBLE | WS_CHILD | WS_BORDER | ES_RIGHT | ES_NUMBER, 200, 50, 100, 20, hwnd_set, Time_id, NULL, NULL);
    HWND mistakes = CreateWindow("edit", "15", WS_VISIBLE | WS_CHILD | WS_BORDER | ES_RIGHT | ES_NUMBER, 200, 90, 100, 20, hwnd_set, Mistakes_id, NULL, NULL);
